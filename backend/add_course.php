...
<head>
    ...
    <style>
        ...
        h2, h3 {
            text-align: center;
            color: #8dbfb7; /* Updated heading color */
        }
        .form-container {
            background: #ffffff;
            padding: 20px;
        ...
        .button, .action-button {
            background-color: #bf8dae; /* Updated button color */
            color: white;
            border: none;
            padding: 12px 20px;
            font-size: 16px;
        ...
        .button:hover, .action-button:hover {
            background-color: #a67a96; /* Darker shade for hover */
        }
        #submitButton {
            background-color: #8dbfb7; /* Updated Add button color */
        }
        #submitButton:hover {
            background-color: #73a59f; /* Darker shade for hover */
        }
        .action-button {
            margin-top: 5px; /* Add margin to create gap between Edit and Delete buttons */
        }
        table {
            width: 100%;
    ...
    <script>
        $(document).ready(function () {
            ...
            window.editCourse = function (id, subjectId, courseName, description, authorName, price, offerPrice, image) {
                ...
                $('#courseName').focus(); // Move cursor to course name input
            ...
            window.deleteCourse = function (id) {
                if (confirm("Are you sure?")) {
                    $.post('process_course.php', { id, action: 'delete' }, function (response) {
                        if (response.success) {
                            $('#row-' + id).remove();
                        }
                    }, 'json');
